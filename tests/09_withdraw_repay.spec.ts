...
import { ecosystem, users, bankKeypairA, oracles, bankKeypairUsdc, verbose, bankKeypairSol } from "./rootHooks";
import { USER_ACCOUNT } from "./utils/mocks";
import { Transaction } from "@solana/web3.js";
import { expectFailedTxWithError, getTokenBalance, assertKeysEqual } from "./utils/genericTests";
import { composeRemainingAccounts, wrappedI80F48toBigNumber } from "@mrgnlabs/mrgn-common";
import { borrowIx, depositIx, repayIx, withdrawEmissionsIx, withdrawIx } from "./utils/user-instructions";
...
describe("Withdraw funds", () => {
    ...
    it("(user 0) withdraws some token A - happy path", async () => {
        ...
        const [userAccBefore, userTokenABefore, vaultTokenABefore] = await Promise.all([
        ...
        if (verbose) {
            console.log(
                "User 0 withdraw " + withdrawAmountTokenA + " token A (" + withdrawExpected.toString() + ") native"
            );
        }
    ...
    it("(user 0) repays all of their USDC debt - happy path", async () => {
        ...
        // Actual should repay amount = liab shares * liab share value
        const actualOwed = 
        ...
        if (verbose) {
            console.log("User 0 repaid entire USDC balance: ~" + actualOwed.toLocaleString());
        }
        ...
        // user loses the liab shares of USDC
        const sharesBefore = wrappedI80F48toBigNumber(
            balancesBefore[1].liabilityShares
        ).toNumber();
        const sharesAfter = wrappedI80F48toBigNumber(
    ...
    it("(user 0) withdraws all token A balance - happy path", async () => {
        ...
        if (verbose) {
            console.log("User 0 withdrew all token A balance: ~" + withdrawExpected.toLocaleString());
        }
        ...
        // user loses the asset shares of token A
    ...
    it("(user 0) restores previous Token A deposits and USDC borrow", async () => {
        const user = users[0];
