import { workspace, Program, getProvider, AnchorProvider, Wallet, BN } from "@coral-xyz/anchor";
import { Marginfi } from "../target/types/marginfi";
import { ecosystem, users, bankKeypairA, verbose, bankKeypairUsdc, marginfiGroup, groupAdmin, oracles, bankKeypairSol } from "./rootHooks";
import { Transaction, AccountMeta } from "@solana/web3.js";
import { createMintToInstruction } from "@solana/spl-token";
import { deriveLiquidityVault } from "./utils/pdas";
import { getTokenBalance, assertI80F48Approx, assertI80F48Equal, assertBNApproximately } from "./utils/genericTests";
import { depositIx, withdrawIx } from "./utils/user-instructions";
import { addBankWithSeed } from "./utils/group-instructions";
import { USER_ACCOUNT } from "./utils/mocks";
import { assert } from "chai";
import { defaultBankConfig, ORACLE_SETUP_PYTH_PUSH, u64MAX_BN } from "./utils/types";
...
describe("Deposit funds", () => {
    ...
    it("(user 1) deposit up to limit - happy path", async () => {
        ...
        // And now user 1 attempts to deposit up to the deposit cap
        const user = users[1];
        ...
        if (verbose) {
            console.log("user 1 Token A before: " + userTokenABefore.toLocaleString());
        }
        ...
        assertI80F48Equal(balances[depositIndex].assetShares, new BN(expected));
        let now = Math.floor(Date.now() / 1000);
