import { AnchorProvider,BN, workspace, getProvider, Program, Wallet } from "@coral-xyz/anchor";
import { Marginfi } from "../target/types/marginfi";
import { ecosystem, marginfiGroup, users, groupAdmin, bankKeypairUsdc, verbose } from "./rootHooks";
import { assertKeyDefault, assertKeysEqual, getTokenBalance, assertBNEqual, assertI80F48Approx } from "./utils/genericTests";
import { groupConfigure, setupEmissions, updateEmissions } from "./utils/group-instructions";
import { Transaction } from "@solana/web3.js";
import { createMintToInstruction } from "@solana/spl-token";
import { deriveEmissionsTokenAccount, deriveEmissionsAuth } from "./utils/pdas";
import { EMISSIONS_FLAG_BORROW_ACTIVE, EMISSIONS_FLAG_LENDING_ACTIVE } from "./utils/types";
...
describe("Lending pool set up emissions", () => {
    ...
    it("(user 1) Add more token B emissions on (USDC) bank - happy path", async () => {
        ...
        assert.equal(adminBBefore - adminBAfter, totalEmissions.toNumber());
        assert.equal(emissionsAccAfter, emissionsAccBefore + totalEmissions.toNumber());
    })
})