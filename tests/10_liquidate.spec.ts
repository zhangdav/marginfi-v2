...
import { ecosystem, users, bankKeypairA, bankKeypairUsdc, oracles, groupAdmin, verbose } from "./rootHooks";
import { CONF_INTERVAL_MULTIPLE, defaultBankConfigOptRaw } from "./utils/types";
import { USER_ACCOUNT } from "./utils/mocks";
import { expectFailedTxWithError, assertI80F48Equal, getTokenBalance, assertKeysEqual, assertBNApproximately } from "./utils/genericTests";
import { composeRemainingAccounts, bigNumberToWrappedI80F48, wrappedI80F48toBigNumber } from "@mrgnlabs/mrgn-common";
...
describe("Liquidate user", () => {
    ...
    it("(user 1) tries to sneak in a bad oracle for itself - should fail", async () => {
        ...
        console.log("liability bank key", liabilityBankKey.toString());
                    ...
                    new Transaction().add(
                        await liquidateIx(liquidator.mrgnProgram, {
                            ...
                            remaining: [
                                ...
                                ...composeRemainingAccounts([
                                    [liabilityBankKey, oracles.fakeUsdc], // sneaky
                                    [assetBankKey, oracles.tokenAOracle.publicKey],
                                ]),
                            ],
                            amount: liquidateAmountA_native,
  ...
  it("(user 1) liquidate user 0 who borrowed USDC against their token A position - happy path", async () => {
    ...
    const sharesA = wrappedI80F48toBigNumber(liquidateeBalances[1].assetShares).toNumber();
    ...
    if (verbose) {
        ...
        console.log("user 1 (liquidator) USDC asset shares: " + wrappedI80F48toBigNumber(liquidatorBalances[0].assetShares).toNumber().toString());
    ...
    const usdcHighPrice = oracles.usdcPrice * (1 + confidenceInterval);
    ...
    assertI80F48Equal(liquidateeBalancesAfter[0].assetShares, wrappedI80F48toBigNumber(liquidateeBalances[0].assetShares).toNumber() -
    ...
    assertI80F48Equal(liquidatorBalancesAfter[0].assetShares, liquidateAmountA_native);
    ...
    assert.approximately(insuranceVaultBalanceAfter, insuranceToBeCollected, insuranceToBeCollected * 0.1);

    if (verbose) {
        console.log("AFTER");
        console.log("liab bank insurance vault after (usdc): " + insuranceVaultBalanceAfter.toLocaleString());
        ...
        console.log("user 0 (liquidatee) USDC liab shares after: " + sharesUsdcAfter.toString());
        ...
        console.log("user 1 (liquidator) USDC asset shares after: " + wrappedI80F48toBigNumber(liquidatorBalancesAfter[0].assetShares).toNumber().toString());
        console.log("user 1 (liquidator) USDC liab shares after: " + wrappedI80F48toBigNumber(liquidatorBalancesAfter[0].liabilityShares).toString());
        console.log("user 1 (liquidator) Token A asset shares after: " + wrappedI80F48toBigNumber(liquidatorBalancesAfter[1].assetShares).toNumber().toString());
        console.log("user 1 (liquidator) Token A liab shares after: " + wrappedI80F48toBigNumber(liquidatorBalancesAfter[1].liabilityShares).toString());
    }
