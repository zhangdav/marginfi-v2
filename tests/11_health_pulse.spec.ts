...
import { composeRemainingAccounts, wrappedI80F48toBigNumber } from "@mrgnlabs/mrgn-common";
import { oracles, users, bankKeypairUsdc, bankKeypairA, verbose, groupAdmin } from "./rootHooks";
...
describe("Health pulse", () => {
    ...
    it("(user 1) health pulse with bad oracle - cache notes the missing price", async () => {
            ...
            new Transaction().add(
                await healthPulse(user.mrgnProgram, {
                    ...
                    remaining: composeRemainingAccounts([
                        [bankKeypairUsdc.publicKey, oracles.fakeUsdc], // sneaky
        ...
        if (verbose) {
            ...
            console.log("index of error: " + cacheAfter.errIndex);
            console.log("prices: ");
        ...
        assert.equal(cacheAfter.flags, HEALTH_CACHE_HEALTHY + HEALTH_CACHE_ENGINE_OK);
        assert.equal(cacheAfter.mrgnErr, 0);
        assert.equal(cacheAfter.internalErr, 6052);
    ...
    it("(user 0) health pulse in now-healthy state - happy path", async () => {
        ...
        if (verbose) {
            ...
            for (let i = 0; i < bals.length; i++) {
                ...
                if (shares != 0) {
                    const bankAcc = await program.account.bank.fetch(bankKey);
                    const config = bankAcc.config;
                    expectedValue = shares * wrappedI80F48toBigNumber(bankAcc.assetShareValue).toNumber() * wrappedI80F48toBigNumber(config.assetWeightInit).toNumber();
                    console.log(" [" + i + "] (asset): " + shares + " of " + bankKey.toString());
                    console.log("  exp value/price: " + expectedValue);
                }
                if (debt != 0) {
                    const bankAcc = await program.account.bank.fetch(bankKey);
                    const config = bankAcc.config;
                    expectedDebt = debt * wrappedI80F48toBigNumber(bankAcc.liabilityShareValue).toNumber() * wrappedI80F48toBigNumber(config.liabilityWeightInit).toNumber();
                    console.log(" [" + i + "] (debt):  " + debt + " in " + bankKey.toString());
                    console.log("  exp value/price: " + expectedDebt);
                }
            }
        ...
        assert.approximately(cacheAfter.timestamp.toNumber(), now, 3);
        assert.equal(cacheAfter.flags, HEALTH_CACHE_HEALTHY + HEALTH_CACHE_ENGINE_OK + HEALTH_CACHE_ORACLE_OK);

        const adjustedAssetPrice = oracles.tokenAPrice * (1.0 - confidence);
        const adjustedLiabPrice = oracles.usdcPrice * (1.0 + confidence);
        
        assert.approximately(bytesToF64(cacheAfter.prices[0]), adjustedAssetPrice, t);
        assert.approximately(bytesToF64(cacheAfter.prices[1]), adjustedLiabPrice, t);
        assert.approximately((expectedValue * adjustedAssetPrice) / 10 ** oracles.tokenADecimals, assetValue.toNumber(), 0.01);
        assert.approximately((expectedDebt * adjustedLiabPrice) / 10 ** oracles.usdcDecimals, liabValue.toNumber(), 0.01);
    })
})