...
import { globalFeeWallet, users, marginfiGroup } from "./rootHooks";
import { accountInit, transferAccountAuthorityIx } from "./utils/user-instructions";
import { ACCOUNT_TRANSFER_FEE, ACCOUNT_DISABLED, I80F48_ZERO } from "./utils/types";
import { assert } from "chai";
import { assertKeyDefault, assertKeysEqual, assertBNEqual, expectFailedTxWithMessage, assertI80F48Equal } from "./utils/genericTests";
...
describe("Transfer account authority", () => {
    ...
    it("(user 0) migrate an account with positions to a new account - happy path", async () => {
        ...
        const newAcc = await program.account.marginfiAccount.fetch(newAccKeypair.publicKey);
        ...
        for (let i = 0; i < newAcc.lendingAccount.balances.length; i++) {
            ...
            assertI80F48Equal(balOld.emissionsOutstanding, balNew.emissionsOutstanding);
