...
pub fn transfer_to_new_account(ctx: Context<TransferToNewAccount>) -> MarginfiResult {
    ...
    emit!(MarginfiAccountTransferToNewAccount {
        header: AccountEventHeader {
            ...
            marginfi_account_authority: ctx.accounts.new_marginfi_account.key(),
...
pub struct TransferToNewAccount<'info> {
    ...
    /// CHECK: WARN: New authority is comletely unchecked
    pub new_authority: UncheckedAccount<'info>,

    /// CHECK: Vaulidated against group fee state cache
    #[account(mut)]
