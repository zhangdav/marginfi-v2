...
async fn marginfi_group_withdraw_fees_and_insurance_fund_as_admin_success(
    ...
    assert_eq!(receiving_account.balance().await, expected_received_balance);
...
#[test_case(BankMint::T22WithFee)]
#[tokio::test]
async fn marginfi_group_withdraw_fees_and_insurance_fund_as_admin_failure(
    bank_mint: BankMint,
) -> anyhow::Result<()> {
    // Setup test executor with non-admin payer
    let mut test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
    ...
    // Unable to withdraw 750 USDC from the fee vault, because the signer is not the admin
    assert_anchor_error!(res.unwrap_err(), ErrorCode::ConstraintHasOne);
...
#[test_case(BankMint::T22WithFee)]
#[tokio::test]
async fn marginfi_group_withdraw_fees_permissionless(bank_mint: BankMint) -> anyhow::Result<()> {
    // Setup test executor with non-admin payer
    let mut test_f = TestFixture::new(Some(TestSettings::all_banks_payer_not_admin())).await;
    ...
    // Use proper des account should succeed
    bank_f
    ...
    let expected_received_balance = fee_vault_balance - transfer_fee;
    assert_eq!(destination_ata.balance().await, expected_received_balance);
