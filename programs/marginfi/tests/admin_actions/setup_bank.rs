...
async fn configure_bank_emode_success(bank_mint: BankMint) -> anyhow::Result<()> {
    ...
    assert_eq!(loaded_bank.emode.flags, 0u64);
    assert_eq!(loaded_bank.emode.emode_tag, empty_emode_tag);
    assert_eq!(loaded_bank.emode.timestamp, timestamp);
    assert_eq!(old_bank.emode.emode_config, loaded_bank.emode.emode_config);
    assert_eq!(old_bank.config, loaded_bank.config);
...
async fn configure_bank_interest_only_success() -> anyhow::Result<()> {
    ...
    let ir_config = marginfi::state::marginfi_group::InterestRateConfigOpt {
        optimal_utilization_rate: Some(I80F48!(0.9).into()),
        plateau_interest_rate: Some(I80F48!(0.5).into()),
        max_interest_rate: Some(I80F48!(1.5).into()),
        insurance_fee_fixed_apr: Some(I80F48!(0.01).into()),
        insurance_ir_fee: Some(I80F48!(0.02).into()),
        protocol_fixed_fee_apr: Some(I80F48!(0.03).into()),
        protocol_ir_fee: Some(I80F48!(0.04).into()),
        protocol_origination_fee: Some(I80F48!(0.05).into()),
    };
