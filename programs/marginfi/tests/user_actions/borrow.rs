...
async fn marginfi_account_borrow_success(
    ...
    assert_eq_with_tolerance!(
        ...
        I80F48::from(-accounted_user_balance_delta),
        1
    );
    ...
    let liability_price_roughly: f64 = get_mint_price(debt_mint);

    let disc: f64 = 0.95;
    assert!(asset_value > deposit_amount * collateral_price_roughly * disc);
    assert!(liab_value > borrow_amount * liability_price_roughly * disc);
    ...
    for (i, bal) in marginfi_account.lending_account.balances.iter().enumerate() {
        ...
        if bal.is_active() {
            ...
            if shares != I80F48::ZERO {
                ...
                assert!(price >= (liability_price_roughly * disc));
            }
        }
    }
...
#[tokio::test]
async fn marginfi_account_borrow_failure_not_enough_colaateral(
    deposit_amount: f64,
    ...
    // Prepare LP's collateral and debt token accounts
    ...
    // Prepare user's collateral and debt token accounts
